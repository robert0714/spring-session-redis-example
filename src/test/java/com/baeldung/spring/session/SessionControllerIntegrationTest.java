...
//        jedis = new Jedis("localhost", 6379);
        jedis = buildReplicaJedis();
        jedis.flushAll();
    }

	protected Jedis buildReplicaJedis() {
		String PASSWORD = "foobared";
		String MASTER_NAME = "master1";
		Set<String> sentinels = new HashSet<String>();
		sentinels.add("10.0.30.101:26379");
		sentinels.add("10.0.30.102:26379");
		Jedis jedis = null;

		try (JedisSentinelPool pool = new JedisSentinelPool(MASTER_NAME, sentinels)) {

			// Fetching connection from pool
			jedis = pool.getResource();
			// "Authenticating...
//			jedis.auth(PASSWORD);
			// auth complete...
			Socket socket = jedis.getClient().getSocket();
			System.out.print("Connected to " + socket.getRemoteSocketAddress());

		} catch (JedisException e) {
			System.out.print("Connection error of some sort!");
